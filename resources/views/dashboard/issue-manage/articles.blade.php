                    ...
                    @foreach ($articles as $article)
                        <tr>
                            ...
                            <td style="display: flex;gap: 1rem">
                                <form class="full-w"
                                    action="{{ route('issues.articles.approve', ['issue_id' => $issue->id, 'article_id' => $article->id]) }}"
                                    method="post">
                                    @csrf
                                    <x-button type="submit" size="sm" class="btn-primary full-w">Publish</x-button>
                                </form>

                                <form class="full-w"
                                    action="{{ route('issues.articles.reject', ['issue_id' => $issue->id, 'article_id' => $article->id]) }}"
                                    method="post">
                                    @csrf
